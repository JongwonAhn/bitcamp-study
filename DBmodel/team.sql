...
-- 여행클럽신고
DROP TABLE IF EXISTS c_notify RESTRICT;
...
-- 여행클럽신고
CREATE TABLE c_notify (
  mno       INTEGER  NOT NULL COMMENT '회원번호', -- 회원번호
  cno       INTEGER  NOT NULL COMMENT '클럽게시판번호', -- 클럽게시판번호
  c_nreason LONGTEXT NOT NULL COMMENT '신고사유', -- 신고사유
  c_nresult INTEGER  NOT NULL COMMENT '처리결과', -- 처리결과
  c_ndate   DATE     NOT NULL COMMENT '신고일자' -- 신고일자
)
COMMENT '여행클럽신고';

-- 여행클럽신고
ALTER TABLE c_notify
  ADD CONSTRAINT PK_c_notify -- 여행클럽신고 기본키
    PRIMARY KEY (
      mno, -- 회원번호
      cno  -- 클럽게시판번호
    );
...
-- 신고게시판
ALTER TABLE b_notify
  ADD CONSTRAINT FK_board_TO_b_notify -- 게시글 -> 신고게시판
    FOREIGN KEY (
      bno -- 게시판id
    )
...
-- 신고게시판
ALTER TABLE b_notify
  ADD CONSTRAINT FK_member_TO_b_notify -- 회원 -> 신고게시판
    FOREIGN KEY (
      mno -- 회원번호
    )
...
-- 여행클럽신고
ALTER TABLE c_notify
  ADD CONSTRAINT FK_c_member_TO_c_notify -- 여행참여자 -> 여행클럽신고
    FOREIGN KEY (
      mno, -- 회원번호
      cno  -- 클럽게시판번호
    )
