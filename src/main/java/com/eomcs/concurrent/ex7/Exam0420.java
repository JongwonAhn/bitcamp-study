...
public class Exam0420 {
  ...
  static class MyRunnable implements Runnable {
    ...
    public void run() {
      try {
        ...
        //
        //          double result = Math.tan(3456.234234) / Math.random(); 
        //
        //        }
  ...
  public static void main(String[] args) {
    ...
    //    바로 스레드를 멈출 기회라고 보고 스레드를 종료를 시도할 것이다.
    // => 이런 이유로 shutdown() 보다는 스레드 종료를 강제하는 효과가 있다.
    // => 작업중인 스레드가 Not Runnable 상태가 아니라면 작업이 끝날 때까지 기다린다.
    // => 다만 작업을 마치고 대기중인 스레드는 즉시 취소한다.
